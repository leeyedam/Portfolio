...
const InfoBar = styled.div`
  ...
  > div {
    ...
    background-color: ${(props) =>  props.type === "left" ? "#07ac40" : "#ffba5a"};
    z-index: 1;
    height: 100%;
...
const Card = styled.div`
  ...
  height: 22px;
  border-radius: 3px;
  background-color: ${(props) => props.color === "yellow" ? "#ffba5a" : "#e74c5b"};
  margin-right: 10px;
  @media (max-width: 720px) {
...
const LiveSoccerMobile = ({ setMobileMenu, mobileMenu ,iframeData , pageBasicImgUrl, footballH2HData , rankData , rankingTab , setRankingTab , contentsTab ,setContentsTab , lineUpData , updateStats , updateScore }) => {
                    ...
                    if(data?.type === 25){
                      return (
                          <>
                            ...
                            <InfoBar type="right" width={!(data.home === 0 && data.away === 0 ) ? Math.floor((data.away / (data.away + data.home)) * 100) : 0}>
                              <div></div>
                            </InfoBar>
                            <div>{data.away}</div>
                          </>
