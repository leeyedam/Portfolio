...
import { TimeStampData } from "../components/apis/api";
...
const MainPage = ({ setRankingModal, setLogin, setSignup, signToggle , multiModalToggle , setMultiModalToggle , changeUserToggle , setChangeUserToggle , check , setCheck , setWithdrawal, userDropData , pageBasicData ,modalToast ,setModalToast , setRequestForLevelUp , timeExtension, setTimeExtension , loginExtenButton , setLoginExtenButton}) => {
  ...
  // TimerHandle(test)
    ...
    const [basketOrder , setBasketOrder] = useState("competition_oldest")
    ...
    const [ scoreSocket, setScoreSocket] = useState()

    // 농구 stats 값 (호버시 모달창)
    const [ basketballStatsSocket, setBasketballStatsSocket] = useState()
    // 농구 score 값 (호버시 모달창)
    const [ basketballScoreSocket, setBasketballScoreSocket] = useState()
    
    const [matchIdData, setMatchIdData] = useState()

    const [nowTime, setNowTime] = useState("")
    useEffect(()=>{
      TimeStampData(setNowTime)
    },[])
    
  const footballStats = () => {
    let [a, b, c, d] = locations.pathname.split('/');
    console.log(c,d)
    if(footballData === ""){
      if(locations.pathname === '/score/football' ||( (c === "watch" && d === "football") || (c === "watch" && d === "basketball") ) || locations.pathname === '/score/basketball'){
        socket.connect();
        socket.on("football_stats" , (data)=>{
          setSportsSocket(data)
        });
        
        socket.on("basketball_stats" , (data)=>{
          setBasketballStatsSocket(data)
        })
  ...
  const footballScore = () => {
    ...
    if(footballData === ""){
      if(locations.pathname === '/score/football' ||((c === "watch" && d === "football") || (c === "watch" && d === "basketball") )|| locations.pathname === '/score/basketball'){
        socket.connect();
        socket.on("football_score" , (data)=>{
          setScoreSocket(data)
        })
        socket.on("basketball_score" , (data)=>{
          setBasketballScoreSocket(data)
        })
      }else if(socket){
        socket.disconnect()
      }
  ...
  useEffect(()=>{
    footballStats()
  },[sportsType , footballData , locations ])
  ...
  // useEffect(()=>{
  //   if(footballData !== "" && locations.pathname === '/score/football' && !sportsSocket ){
  //     const socket = io.connect("http://23.227.182.50:8500" , {
  //       transports : ['websocket']
  //     })
  //     if(!sportsSocket){
  //       let [a, b, c, d] = locations.pathname.split('/');
  //       // if(locations.pathname === '/score/football' ||( c === "watch" && d === "football")){
  //       if(locations.pathname === '/score/football'){
  //         console.log("??")
  //         socket.connect();
  //         socket.on("football_stats" , (data)=>{
  //             setSportsSocket(data)
  ...
  //     }
  //   }
  // },[sportsType , footballData , locations , sportsSocket ])
            ...
            <Route
              ...
              element={
                <LiveScoreFootballPage
                  ...
                  order={order}
                  nowTime={nowTime}
                  sportsSocket={sportsSocket}
                  setSportsSocket={setSportsSocket}
            ...
            <Route
              ...
              element={
                <LiveScorebasketball
                  ...
                  mobileMenu={mobileMenu}
                  pageBasicData={pageBasicData}
                  setBasketballData={setBasketballData}
                  ...
                  setOpenSelect={setOpenSelect}
                  order={order}
                  basketballStatsSocket={basketballStatsSocket}
                  basketballScoreSocket={basketballScoreSocket}
                ></LiveScorebasketball>
              }
            />
            <Route
