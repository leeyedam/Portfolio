...
import { BasicData, ScheduleDataBasketball, ScoreCount } from "../../../components/apis/api";
// import io from 'socket.io-client'
import LiveScorebasketballMobile from "./LiveScorebasketballMobile";
...
const LiveScorebasketball = ({ setMobileMenu, mobileMenu , sportsType, setSportsType , basketballData , setBasketballData , nowTime , openSelect, setOpenSelect , order , pageBasicData , basketballStatsSocket , basketballScoreSocket}) => {
  ...
  // ScheduleDataBasketball()
  // setBasketball
  
  // useEffect(()=>{
  //   if(sportsSocket !== undefined){
  //     // setUpdateStats(JSON.parse(updatedOSArray()))
  //     ScheduleDataBasketball(sportsSocket && JSON.parse(sportsSocket))
  //   }
  // },[sportsSocket])
  ...
  useEffect(()=>{
    ...
    ScheduleDataBasketball(setBasketballData , tab === 1 ? "live" : (tab === 2 ? "end" : tab === 3 && "schedule")  , JSON.stringify(dateFormat).replace(/\./g , "").replace(/\"/g , "") , order )
  },[tab ,date ,openSelect , nowTime , order])

  const [basketballScore, setBasketballScore] = useState()

  useEffect(()=>{
    if(basketballScoreSocket !== undefined){
      // setUpdateStats(JSON.parse(updatedOSArray()))
      setBasketballScore(basketballScoreSocket && JSON.parse(basketballScoreSocket))
    }
  },[basketballScoreSocket])
  ...
  return (
    <Wrapper>
      <ContentsContainer>
          ...
          <TopBarContainer>
            <TabContainer>
              <Tab active={tab === 1} onClick={() => setTab(1)}>
                ...
                <TabText active={tab === 1}>({basketballData?.data_count})</TabText>
              </Tab>
            ...
            basketballData?.data?.map((data, index) => {
              return (
                  <>
                    ...
                    <LeagueInfoCotainer
                      height="81px"
                      onClick={() => navigate(`/live/watch/basketball/${data?.match_id && data?.match_id}`)}
                    >
                        ...
                        <BasketballTime>
                          {
                            ...
                            <>
                                <TimeBackground type={ data.status_id === 1 ?
                                  "whiteGray" 
                                  :
                                  (
                              ...
                              <span>{(getTimeStringSeconds(data.remaining_time))}</span>
                            </>
                          }
                          {
                            basketballScore?.id === data?.match_id &&
                            <>
                                <TimeBackground type={ basketballScore && basketballScore?.result[1] === 1 ?
                                  "whiteGray" 
                                  :
                                  (
                                    (basketballScore &&
                              ...
                              <span>{(getTimeStringSeconds(basketballScore?.result[2]))}</span>
                            </>
                          }
                        </BasketballTime>
                      </BasketballTimeContents>
                        ...
                        <BasketballScore>
                          <div>
                            <BasketballScoreContent>
                              {
                                basketballScore?.id !== data?.match_id &&
                                data.away_team_info.score_1
                              }
                              
                              {
                                basketballScore?.id === data?.match_id &&
                                basketballScore?.result[4][0]
                              }
                            </BasketballScoreContent>
                            <BasketballScoreContent>
                              {
                                basketballScore?.id !== data?.match_id &&
                                data.away_team_info.score_2
                              }
                              
                              {
                                basketballScore?.id === data?.match_id &&
                                basketballScore?.result[4][1]
                              }
                            </BasketballScoreContent>
                            <BasketballScoreContent>
                              {
                                basketballScore?.id !== data?.match_id &&
                                data.away_team_info.score_3
                              }
                              
                              {
                                basketballScore?.id === data?.match_id &&
                                basketballScore?.result[4][2]
                              }
                            </BasketballScoreContent>
                            <BasketballScoreContent>
                              {
                                basketballScore?.id !== data?.match_id &&
                                data.away_team_info.score_4
                              }
                              
                              {
                                basketballScore?.id === data?.match_id &&
                                basketballScore?.result[4][3]
                              }
                            </BasketballScoreContent>
                            <BasketballScoreContent>
                              {
                                basketballScore?.id !== data?.match_id &&
                                data.away_team_info.score_overtime
                              }
                              
                              {
                                basketballScore?.id === data?.match_id &&
                                basketballScore?.result[4][4]
                              }
                            </BasketballScoreContent>
                          ...
                          <BasketballScoreContent>
                              {
                                basketballScore?.id !== data?.match_id &&
                                data.home_team_info.score_1
                              }
                              
                              {
                                basketballScore?.id === data?.match_id &&
                                basketballScore?.result[3][0]
                              }
                            </BasketballScoreContent>
                            <BasketballScoreContent>
                              {
                                basketballScore?.id !== data?.match_id &&
                                data.home_team_info.score_2
                              }
                              
                              {
                                basketballScore?.id === data?.match_id &&
                                basketballScore?.result[3][1]
                              }
                            </BasketballScoreContent>
                            <BasketballScoreContent>
                              {
                                basketballScore?.id !== data?.match_id &&
                                data.home_team_info.score_3
                              }
                              
                              {
                                basketballScore?.id === data?.match_id &&
                                basketballScore?.result[3][2]
                              }
                            </BasketballScoreContent>
                            <BasketballScoreContent>
                              {
                                basketballScore?.id !== data?.match_id &&
                                data.home_team_info.score_4
                              }
                              
                              {
                                basketballScore?.id === data?.match_id &&
                                basketballScore?.result[3][3]
                              }
                            </BasketballScoreContent>
                            <BasketballScoreContent>
                              {
                                basketballScore?.id !== data?.match_id &&
                                data.home_team_info.score_overtime
                              }
                              
                              {
                                basketballScore?.id === data?.match_id &&
                                basketballScore?.result[3][4]
                              }
                            </BasketballScoreContent>
                            {/* <BasketballScoreContent active={true}>
                              2
        ...
        <LiveScorebasketballMobile basketballData={basketballData} openSelect={openSelect} setOpenSelect={setOpenSelect} order={order} pageBasicImgUrl={pageBasicImgUrl} tab={setTab} setTab={setTab} basketballScore={basketballScore}></LiveScorebasketballMobile>
