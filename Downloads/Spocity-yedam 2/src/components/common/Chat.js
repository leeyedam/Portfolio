...
import DeleteChatting from "../popup/DeleteChatting";
...
const Wrapper = styled.div`
  ...
  height: 100vh;
  filter: ${(props) =>
    props.reportPopup || props.deletePopup ? "brightness(0.2)" : ""};
...
const AdminSeepchBubble = styled.div`
  width: 266px;
...
const BlockChattingAlert = styled.div`
  display: flex;
...
const Chat = () => {
  ...
  const [openReportSelect, setOpenReportSelect] = useState({
    open: false,
    text: "신고이유 선택",
  });
  ...
  const clickUser = (idx) => {
    setUserMenuIndex(idx);
    setTargetUser(idx);
  };
  const makeDeleteMessage = () => {
  ...
  const tmpList = [
    {
      tier: "Bronze",
      nickname: "브론즈",
      text: "아 맞네 임성빈 음주운전이네 ... 대화가 길어질 때 / 문장이 길어져서 2줄 이상일 때 / 이런식으로 나타나게 됩니다.",
    },
    {
      tier: "Silver",
      nickname: "실버",
    ...
    },
    {
      tier: "Admin",
      nickname: "관리자",
      text: "아 맞네 임성빈 음주운전이네 ... 아 맞네 임성빈 음주운전이네 ... 아 맞네 임성빈 음주운전이네 ... 아 맞네 임성빈 음주운전이네 ...",
    },
    {
      tier: "Bot",
      nickname: "스포시티봇",
  ...
  const tierConversion = (tier) => {
    switch (tier) {
      case "Bronze":
        return ["B", "#9a624e", "rgba(255, 255, 255, 0.3)"];
      case "Silver":
        return ["S", "#6690a0", "rgba(255, 255, 255, 0.3)"];
      case "Gold":
        return ["G", "#ba963e", "rgba(240, 185, 52, 0.4)"];
      case "Platinum":
        return ["P", "#aba349", "rgba(255, 247, 160, 0.4)"];
      case "Diamonds":
        return ["D", "#665eb3", "rgba(179, 145, 255, 0.4)"];
      case "Admin":
        return [
          "A",
          "linear-gradient(to right, #00e03b, #00a4f0);",
          "rgba(0, 221, 75, 0.4);",
        ];
      case "Bot":
        return ["BOT", "#007bba", "rgba(0, 168, 255, 0.5)"];
      default:
        return "";
    }
  };
        ...
        if (reportPopup) {
          return (
            <ReportUser
              ...
              openReportSelect={openReportSelect}
              setOpenReportSelect={setOpenReportSelect}
            />
          );
        } else if (deletePopup) {
          return (
            <DeleteChatting
              ...
              makeDeleteMessage={makeDeleteMessage}
            />
          );
        }
      })()}

      <Wrapper reportPopup={reportPopup} deletePopup={deletePopup}>
        <TabContainer>
          <div
            style={{
              width: "110px",
        ...
        {tab === 1 && (
          <>
            ...
            >
              <WelcomeMessage>
                홍길동님, 채팅방에 오신 것을 환영합니다!
                <br /> 매너있는 채팅 문화를 만들어 주세요.
              </WelcomeMessage>
                      ...
                      <div style={{ display: "flex" }}>
                        ...
                        <div>
                          ...
                          >
                            {item.tier === "Admin" ? (
                              <AdminClass
                                background={tierConversion(item.tier)[1]}
                              >
                            ...
                            <ClassText
                              ...
                              color={tierConversion(item.tier)[2]}
                              onClick={() => clickUser(idx)}
                              className="nickname"
                            >
                              {item.nickname} :
                              {userMenuIndex === idx &&
                                (item.tier === "Diamonds" ? (
                                  <AdminMenuContainer className="nickname">
                          ...
                          <div>
                            {item.tier === "Admin" ? (
                              <AdminSeepchBubble>{item.text}</AdminSeepchBubble>
                            ) : item.deleteMessage ? (
                              <div
                                style={{
                                  display: "flex",
              ...
              <BlockChattingAlert>
                ...
                />
                <span>홍길동님이 10분 동안 채팅금지 처리 되었습니다</span>
              </BlockChattingAlert>
              {!isScrollBottom && (
                <img
